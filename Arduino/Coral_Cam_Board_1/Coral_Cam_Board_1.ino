...
uint8_t image_compression = 3; //Compression, 1 (smallest image) to 3 (largest image)

//*****************************Dawn-to-Dusk Schedule****************************************
// Cameras only fire during daylight hours to save power and skip dark frames.
// Hours are 24-hour clock, based on the RTC time.
// For continuous, around-the-clock operation set daylight_start = 0 and daylight_end = 23.
uint8_t daylight_start = 7;  //Hour (24hr) to START taking images at dawn
uint8_t daylight_end   = 17; //Hour (24hr) to STOP taking images at dusk
//****************************************************************************************
...
void setup()
{
  ...
  //Set absolute RTC time here. Recommend making 5 min after expected code download, then resetting at this time.
  rtc1.begin();

  DateTime now = rtc2.now();

  rtc1.setTime(now.hour(), now.minute(), now.second());  // HH MM SS
  rtc1.setDate(now.day(), now.month(), now.year()- 2000);  // DD MM YY
...
void loop()
{
  ...
  //Only flash the battery-status LED for the first ~25s of AWAKE time after a power-up
  //(millis() freezes during standby sleep on the Feather M0, so this spans roughly the first
  //camera cycle). Long enough to confirm the board started after a mag-switch power cycle,
  //then it stays dark the rest of the deployment to save battery. --Per Wayne
  if (millis() < 25000) {
    FlashStatus();
  }

  //Take a burst only when it's an image_interval boundary AND we're within the
  //dawn-to-dusk window. Outside daylight the board still wakes every 30s to pet
  //the watchdog and check battery, but skips the camera block (saves power, no
  //dark frames). See daylight_start / daylight_end near the top of the sketch.
  if (minutes_past_midnight() % image_interval == 0 &&
      rtc1.getHours() >= daylight_start &&
      rtc1.getHours() <= daylight_end) {
